...
class SpeechDataset(Dataset):
    def __init__(
        ...
        self.mode = mode
        self.transcript = details["text"]
        self.transcript_dict = dict(zip(self.files, self.transcript))
        if self.mode != "test":
            self.labels = details["label"]
            self.labels_dict = dict(zip(self.files, self.labels))
            self.final_labels = {}
            for k in self.labels_dict:
                if len(self.transcript_dict[k].split()) > 50:
                    continue
    ...
    def __getitem__(self, index):
        ...
        transcript = self.transcript_dict[wav_name]
...
class EmotionClassifier(nn.Module):
    ...
    def forward(self, samples, audio, text):
        samples["text"] = ["Describe emotion of the speaker in one word "+"<Transcript> " + text[i] for i in range(len(text))]
        length = [len(x.split()) for x in samples["text"]]
...
def create_dataset(mode, bs=8):
    transcript_train_path = "train_new_dict.json"

    transcript_val_path = "valid_new_dict.json"

    if mode == 'train':
        f = open(transcript_train_path)
        details = json.load(f)
        f.close()

        mode = "train"
    elif mode == 'val':
        f = open(transcript_val_path)
        details = json.load(f)
        f.close()
        mode = "train"
    else:
...
def train():
    train_loader = create_dataset("train", 4)
    val_loader = create_dataset("val", 4)
    num_classes = 8
    ...
    for e in range(20):
        ...
        for i, data in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=True):
                final_out = model(samples, aud, text)
        ...
        with torch.no_grad():
            for i, data in enumerate(val_loader):
                ...
                samples = {"spectrogram": spectrogram,"raw_wav": aud,"padding_mask": padding_mask}
                with torch.cuda.amp.autocast(enabled=True):
                    val_out = model(samples, aud, text)
        ...
        if val_f1 > final_val_loss:
            ...
            for k in list(state_dict.keys()):
                if k in param_grad_dic.keys() and not param_grad_dic[k]:
                    ...
                    del state_dict[k]
            torch.save(state_dict, "salmonn13b_both_vs.pth")
...
def test():
    ...
    salmonn_model = SALMONN.from_config(cfg.config.model)
    model = EmotionClassifier(salmonn_model, 1024, num_classes)
    model.load_state_dict(torch.load("salmonn_both_vs.pth"), strict=False)
    model.to(device)
    model.eval()
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(test_loader)):
            ...
            text = data["transcript"]
            with torch.cuda.amp.autocast(enabled=True):
                test_out = model(samples, aud, text)
    ...
    df.to_csv("salmonn_both_test_vs.csv", index=False)
